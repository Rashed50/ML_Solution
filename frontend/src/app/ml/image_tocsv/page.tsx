...
export default function ImageUploader() {
  ...
  const upload_file = async (file:File,bbox) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}image-to-csv/`, {
        method: 'POST',
  ...
  const handleMouseClick = (e) => {
    // e contains all the necessary data
    // ...
    bbox[touch_counter++] = e.nativeEvent.offsetX;
    bbox[touch_counter++] = e.nativeEvent.offsetY;
     

   // alert(e.nativeEvent.offsetX);
    if(bbox.length == 4){
      bbox[2] = bbox[2]-bbox[0] 
      bbox[3] = bbox[3]-bbox[1] 
      upload_file(selectedFile,bbox);
      bbox = [];
        ...
        <div className="flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-gray-200">
          ...
          <div className="md:w-1/3 p-6 flex flex-col items-center justify-start bg-gray-50">
            ...
            <div className="h-64 w-full flex items-center justify-center border border-gray-300 rounded-lg overflow-hidden bg-white">
              {previewUrl ? (
                <img     src={previewUrl}  alt="Original Preview"  className="max-h-full w-auto object-contain" width={220} height={270}
